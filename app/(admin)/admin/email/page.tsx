...
// ── Types (field names match Firestore exactly) ──────────────────────────────
...
type Tab = "non-lette" | "tutte" | "inviate";
...
export default function EmailPage() {
  ...
  const [tab, setTab]           = useState<Tab>("non-lette");
  ...
  useEffect(() => {
    async function load() {
      try {
        // ⚠ orderBy("receivedAt") — field name from Firestore schema
  ...
  async function handleSelect(e: EmailFS) {
    ...
    if (e.letta) return;
    setEmails((prev) => prev.map((em) => em.id === e.id ? { ...em, letta: true } : em));
    try {
  ...
  async function handleGeneraAI() {
    ...
    try {
      ...
      if (!res.ok) throw new Error("Errore CF");
      const data = await res.json() as { reply?: string };
      const reply = data.reply ?? "";
      setRisposta(reply);
      setSelected((prev) => prev ? { ...prev, Risposta_suggerita: reply } : prev);
      toast.success("Risposta AI generata");
    } catch {
  ...
  async function handleSend() {
    ...
    try {
      await fetch(
        ...
        { method: "POST", headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ emailId: id, reply: risposta.trim() }) }
      );
      await updateDoc(doc(db, "Emails", id), { Risposto: true, direzione: "inviata", letta: true });
      setEmails((prev) => prev.map((em) => em.id === id ? { ...em, Risposto: true, direzione: "inviata" } : em));
      setSelected(null);
      setRisposta("");
  ...
  const nonLette = useMemo(() => emails.filter((e) => e.direzione !== "inviata" && !e.letta), [emails]);
  const tutte    = useMemo(() => emails.filter((e) => e.direzione !== "inviata"), [emails]);
  const inviate  = useMemo(() => emails.filter((e) => e.direzione === "inviata"), [emails]);

  const baseList = tab === "non-lette" ? nonLette : tab === "tutte" ? tutte : inviate;

  const lista = useMemo(() => {
    if (!search) return baseList;
    const q = search.toLowerCase();
    return baseList.filter((e) =>
      [e.from, e.subject, e.body].join(" ").toLowerCase().includes(q)
    );
  }, [baseList, search]);

  const tabs: { key: Tab; label: string }[] = [
    { key: "non-lette", label: `Non lette (${nonLette.length})` },
    ...
    <div className="space-y-4">
      ...
      <div className="flex gap-3" style={{ height: "calc(100vh - 200px)", minHeight: 520 }}>
        ...
        <Card padding="sm" className="flex flex-col overflow-hidden"
          ...
          {/* Tabs */}
          <div className="flex gap-1 mb-3 p-1 rounded-xl w-fit flex-shrink-0"
          ...
          <div className="flex-1 overflow-y-auto">
            {loading ? <ListSkeleton /> : lista.length === 0 ? (
              ...
              lista.map((e) => {
                const iniz = iniziali(e.from);
                const isUnread = !e.letta && e.direzione !== "inviata";
                const isSelected = selected?.id === e.id;
                return (
                  <div key={e.id} onClick={() => handleSelect(e)} className="cursor-pointer"
                    style={{ borderBottom: "1px solid var(--border)" }}>
                    <div className="flex items-start gap-2.5 py-3 px-1 rounded-xl transition-colors hover:bg-[#FFFDF0]"
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center justify-between gap-1">
                          <p className="text-xs truncate"
                            style={{ fontFamily: "var(--font-montserrat)", fontWeight: isUnread ? 700 : 500, color: "var(--text-primary)" }}>
                            {e.from ?? "—"}
                          </p>
                          <div className="flex items-center gap-1 flex-shrink-0">
                            {isUnread && <div className="w-1.5 h-1.5 rounded-full" style={{ background: "var(--brand)" }} />}
                        ...
                        <div className="flex items-center gap-1.5 mt-0.5">
                          ...
                          {e.tipologia && (
                            <Badge variant={TIPOLOGIA_VARIANT[e.tipologia] ?? "neutral"}
                              className="flex-shrink-0">{e.tipologia}</Badge>
                          )}
                        </div>
                      </div>
                    </div>
        ...
        {selected && (
          <Card padding="sm" className="flex flex-col flex-1 min-w-0 overflow-hidden">
            ...
            <div className="flex items-start justify-between mb-3 flex-shrink-0">
              <div className="flex-1 min-w-0 pr-2">
                ...
                <p className="text-xs mt-1" style={{ color: "var(--text-muted)", fontFamily: "var(--font-montserrat)" }}>
                  ...
                  {selected.to && <> · A: <span style={{ color: "var(--text-secondary)" }}>{selected.to}</span></>}
                  {selected.receivedAt && ` · ${formatData(selected.receivedAt)}`}
                </p>
              </div>
              <button onClick={() => setSelected(null)} className="p-1.5 rounded-xl flex-shrink-0"
                style={{ background: "var(--bg-primary)", border: "1px solid var(--border)" }}>
            ...
            {/* Body */}
            <div className="flex-1 rounded-xl p-3 mb-3 text-sm leading-relaxed overflow-y-auto"
              style={{ background: "var(--bg-primary)", border: "1px solid var(--border)",
            ...
            {/* AI + Reply — only for received emails */}
              ...
              <div className="flex-shrink-0 space-y-2">
                ...
                <div className="flex items-center justify-between">
                  <p className="text-[10px] font-bold uppercase tracking-widest"
                    style={{ color: "var(--text-muted)", fontFamily: "var(--font-montserrat)" }}>
                    {selected.Risposta_suggerita ? "Risposta suggerita" : "Risposta rapida"}
                  </p>
                  <button onClick={handleGeneraAI} disabled={generando}
                    ...
                    style={{ background: "#f9fafb", border: "1px solid var(--border)", fontFamily: "var(--font-montserrat)" }}>
                    {generando ? <Loader2 size={11} className="animate-spin" /> : <Sparkles size={11} />}
                    Genera AI
                  </button>
                </div>

                <textarea rows={4} value={risposta} onChange={(e) => setRisposta(e.target.value)}
                  placeholder="Scrivi una risposta…"
                  className="w-full rounded-xl p-3 text-sm outline-none resize-none"
                  style={{ background: "var(--bg-primary)", border: "1px solid var(--border)",
                    fontFamily: "var(--font-montserrat)", color: "var(--text-primary)" }} />

                <button onClick={handleSend} disabled={sending || !risposta.trim()}
            ...
            {selected.direzione === "inviata" && (
              <div className="rounded-xl p-3 flex-shrink-0 text-sm"
                style={{ background: "#F0FDF4", border: "1px solid #BBF7D0",
                  fontFamily: "var(--font-montserrat)", color: "#065F46" }}>

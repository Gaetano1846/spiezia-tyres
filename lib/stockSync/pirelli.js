// Sync stock Pirelli OCP (Fase 9-bis → Fase 3 decommissioning Firebase,
// 2026-07-14) — port 1:1 della Cloud Function `processPirelliOCP`
// (crm-3iuocs, europe-west8), sorgente reale riscaricato da GCP. Stessa
// logica: mailbox IMAP dedicata (pirelli.ocp@spieziatyres.it) → ultima email
// con allegato XLSX/CSV "Stock OCP" → parse → diff contro cache dell'ultimo
// stato noto → scrittura di `stock_ocp` su public.prodotti (match su `cai`,
// Postgres — non più Firestore).
//
...
//    l'email come letta (\Seen) e NON scrive su Postgres/cache — marcare
//    un'email come letta è un effetto collaterale sulla mailbox reale, va
//    evitato durante la finestra di verifica pre-cutover.
//  - `cai` su public.prodotti è sempre text (i codici sorgente sono già
//    stringhe via String(ip).trim()) — niente più bisogno di rilevare il
//    tipo del campo come si faceva con Firestore (detectCaiType, rimosso).
...
function collectAttachmentParts(struct, acc = []) {
  ...
  // imapflow espone `disposition` come stringa ("attachment"/"inline", non un
  // oggetto con .type) e il filename sotto `dispositionParameters.filename`
  // (non `disposition.params.filename`) o, in fallback, `parameters.name`
  // (non `params.name`) — verificato contro l'output reale del server
  // (bodyStructure dump 2026-08-19). I nomi sbagliati facevano fallire questo
  // fast path SEMPRE, silenziosamente: ogni run degradava al fallback
  // full-MIME-parse da 45s (ultimi 10 messaggi) senza nessun log che lo
  // segnalasse — innocuo finché la mailbox ha poco traffico, ma fragile.
  const dispType = typeof struct.disposition === "string" ? struct.disposition.toLowerCase() : "";
...
async function updateStockOcp(changedPairs) {
  const db = getDb();
  if (!db) throw new Error("Postgres non configurato");

  let totalMatched = 0;
  const CHUNK = 500;
  for (let i = 0; i < changedPairs.length; i += CHUNK) {
    const chunk = changedPairs.slice(i, i + CHUNK);
    const values = [];
    const placeholders = chunk.map(({ ipCode, stock }, idx) => {
      values.push(ipCode, stock);
...
export async function runPirelliOcpSync(opts = {}) {
  ...
  const updatedCount = changed.length ? await updateStockOcp(changed) : 0;

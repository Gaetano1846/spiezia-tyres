...
// da 2/3 colonne), fa match sui clienti per `Partita_Iva` (con fallback
// rimuovendo gli zeri iniziali, necessario perché il CSV a volte non li ha).
//
// Fix split-brain Fido: il gate di blocco checkout legge/scrive
// core.clienti.fido/fido_residuo su Postgres (lib/clientiDb.ts). Questo sync
// scriveva invece SOLO su Firestore `Clienti`, quindi un aggiornamento fido
// legittimo dalla banca via FTP non raggiungeva mai il valore usato realmente
// per bloccare/sbloccare gli ordini. Ora scrive `fido`/`fido_residuo` su
// core.clienti (match per `partita_iva`); il bridge esistente (trigger
// trg_bridge_outbox già presente su core.clienti — non toccato qui) propaga
// automaticamente a Firestore per la UI CRM FlutterFlow legacy che lo legge
// ancora da lì. Non scriviamo più `Last_Fido_Update` (campo Firestore-only,
// non letto da nessuna UI, senza colonna equivalente su core.clienti — il
// timestamp dell'ultimo aggiornamento resta comunque tracciato da
// `core.clienti.updated_at`, aggiornato automaticamente dal trigger esistente
// `trg_touch`).
//
...
//  - Modalità `dryRun`: fetch/parse/diff ma nessuna scrittura Postgres/cache.
...
/** Match clienti per Partita_Iva su core.clienti (Postgres) — sostituisce la query Firestore. */
async function queryClientsByPartitaIvaPg(pool, partitaIvaList) {
  ...
  for (let i = 0; i < unique.length; i += 200) {
    ...
    const { rows } = await pool.query(
      `SELECT id, partita_iva FROM core.clienti WHERE partita_iva = ANY($1)`,
      [chunk]
    );
    for (const row of rows) {
      const original = backMap.get(row.partita_iva);
      if (original && !results[original]) {
        results[original] = { id: row.id, partitaIva: row.partita_iva };
      }
    }
  }
  return results;
...
export async function runFidoSync(opts = {}) {
  ...
  const pool = getDb();
  if (!pool) throw new Error("Postgres non configurato (DATABASE_URL mancante)");
  const pgClients = await queryClientsByPartitaIvaPg(
    pool,
  ...
  for (const c of toCheck) {
    const match = pgClients[c.partitaIva];
    if (!match) continue;
    try {
      await pool.query(
        `UPDATE core.clienti SET fido = $1, fido_residuo = $2 WHERE id = $3`,
        [c.fido, c.fidoResiduo, match.id]
      );
      updatedCount++;
    } catch (err) {

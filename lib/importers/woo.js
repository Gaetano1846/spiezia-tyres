...
import { amazonGetToken, amazonGetOrderItemsWithSku } from "../marketplace/sdk";
...
// Mercati Amazon coperti dal sync prodotti su public.prodotti (colonne
// amazon_sku_<cc>/amazon_asin_<cc> — vedi resolveProdottoSkuByAmazonId sotto).
// Whitelist esplicita: questi valori finiscono interpolati nel nome colonna
// SQL (i driver pg non parametrizzano identificatori), MAI accettare un
// country code non in questa lista.
const AMAZON_SYNCED_COUNTRIES = new Set(["it", "fr", "de", "es", "nl", "be"]);

// Risolve lo SKU reale via il sync Amazon->Postgres (public.prodotti.amazon_sku_*
// / amazon_asin_*), quando SellerSKU/ASIN dell'ordine Amazon combaciano con
// un'inserzione che conosciamo per quel mercato. Match esatto (non fuzzy):
// se non trova nulla ritorna null, mai un tentativo "quasi giusto".
async function resolveProdottoSkuByAmazonId(pool, country, { sellerSku, asin }) {
  const cc = String(country || "").toLowerCase();
  if (!pool || !AMAZON_SYNCED_COUNTRIES.has(cc) || (!sellerSku && !asin)) return null;
  const { rows } = await pool.query(
    `SELECT id FROM public.prodotti WHERE amazon_sku_${cc} = $1 OR amazon_asin_${cc} = $2 LIMIT 2`,
    [sellerSku || null, asin || null]
  );
  // Ambiguo (non dovrebbe capitare, sono ID univoci per mercato) → non indovinare.
  return rows.length === 1 ? rows[0].id : null;
}

// NB: risolve DELIBERATAMENTE via Firestore (non public.prodotti) — stesso
// motivo documentato in tyre24PgWrite.js::resolveArticlesPg. L'unica eccezione
// è il lookup Amazon sopra, che DEVE passare da public.prodotti perché è lì
// che vive il sync amazon_sku_*/amazon_asin_* (mai esistito su Firestore) —
// una volta trovato lo SKU però si torna su Firestore per Ref/Titolo/PFU,
// stesso identico path del match diretto per SKU qui sotto.
//
// `amazonContext` (opzionale): { pool, amazonOrderId, country }. Passato solo
// per ordini `created_via === "amazon"` — vedi processOrder. Copre il caso
// (~11% delle righe Amazon) dove l'integrazione Amazon->WooCommerce non porta
// né SKU né product_id reale (arriva letteralmente product_id: 0): l'unico
// dato affidabile a quel punto è l'ordine Amazon stesso, richiesto alla
// SP-API una sola volta per l'intero ordine (token+orderItems pigri, solo se
// almeno una riga ne ha davvero bisogno) e poi abbinato riga per riga per
// titolo esatto (stesso testo, stessa fonte).
async function processArticles(db, lineItems, amazonContext) {
  const articoli = [];
  let amazonItems = null; // caricati pigri, solo se serve davvero
  ...
  for (const item of lineItems) {
    ...
    if ((!prodottoQuery || prodottoQuery.empty) && amazonContext) {
      if (amazonItems === null) {
        try {
          const accessToken = await amazonGetToken();
          amazonItems = await amazonGetOrderItemsWithSku(accessToken, amazonContext.amazonOrderId);
        } catch (err) {
          console.error(`[woo] Amazon SP-API orderItems fallito per ${amazonContext.amazonOrderId}:`, err.message);
          amazonItems = [];
        }
      }
      const amazonItem = amazonItems.find((a) => a.title === item.name);
      if (amazonItem) {
        const pgSku = await resolveProdottoSkuByAmazonId(amazonContext.pool, amazonContext.country, amazonItem);
        if (pgSku) {
          const skuQuery = await db.collection("Prodotti").where("SKU", "==", pgSku).limit(1).get();
          if (!skuQuery.empty) {
            prodottoQuery = skuQuery;
            amazonMatched = true;
          }
        }
      }
    ...
    const fsExtra = { WC_ProductID: item.product_id, WC_ItemID: item.id, ...(amazonMatched ? { AmazonSyncMatched: true } : {}) };
...
export async function processWooOrder(db, wcOrder, dryRun) {
  ...
  // wcOrder.number è già l'AmazonOrderId reale per created_via==="amazon"
  // (vedi resolveOrderDocId sopra, usato per costruire l'id "AMZ<number>").
  const amazonContext = wcOrder.created_via === "amazon"
    ? { pool: getDb(), amazonOrderId: wcOrder.number, country: wcOrder.billing.country }
    : null;

  const clienteId = await processCustomer(wcOrder);
  const articoli = await processArticles(db, wcOrder.line_items, amazonContext);

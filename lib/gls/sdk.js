...
//
// Porting 1:1 della logica originale. Le differenze rispetto alla CF sono gli
// adattatori di I/O:
//   • Ordini/Spedizioni → Postgres (core.ordini/b2b.spedizioni, Fase 4
//     migrazione Spedizioni/GLS) invece di Firestore diretto — il bridge
//     esistente propaga verso Firestore per il CRM Flutter legacy.
//   • Storage    → saveBufferToPrivate (lib/storage.ts, disco locale VPS
//     /opt/spiezia/storage/private/, decommissioning finale Firebase
//     2026-07-19) invece di Firebase Storage (crm-3iuocs.appspot.com) — le
//     etichette contengono dati cliente (nome/indirizzo destinatario), quindi
//     private (sessione CRM richiesta via app/api/files/[...path]), non
//     pubbliche come banner/loghi/immagini disegno.
...
// domestico, calcolo colli/peso, merge PDF, generazione ZPL) è invariata — SOLO
// i punti di lettura/scrittura dell'ordine sono stati convertiti, con lo
// STESSO ordine rispetto alle chiamate reali all'API GLS (mai riordinato: ogni
// azione GLS è fatturata e non annullabile, la scrittura di stato deve sempre
// seguire, mai precedere, l'esito reale).

import { PDFDocument } from "pdf-lib";
import xml2js from "xml2js";
import { saveBufferToPrivate } from "../storage";
import { getOrdine, updateOrdineGls } from "../ordiniDb";
import {
  createSpedizioniEntries as pgCreateSpedizioniEntries,
  updateSpedizioniStatus as pgUpdateSpedizioniStatus,
} from "../spedizioniDb";
import { processMarketplaceAction } from "../marketplace/sdk";

// Ricostruisce un oggetto "orderData"-shaped (stessa forma del vecchio
// Firestore doc.data()) da Postgres, così la logica sottostante di ogni
// funzione GLS (calcolo colli/peso, indirizzi, ecc.) resta invariata — solo
// il punto di lettura cambia. FsExtra contiene i campi GLS_/ZPL_ con lo
// stesso nome esatto usato dal writer Firestore storico (mai un replace, solo
// un passthrough — vedi mapping/ordini.mjs::unmapped).
async function orderDataFromPg(ordiniId) {
  const ordine = await getOrdine(ordiniId);
  if (!ordine) return null;
  const fsExtra = ordine.FsExtra || {};
  return {
    ID: ordine.ExternalOrderId,
    Source: ordine.Source,
    Pagamento: ordine.Pagamento,
    Totale: ordine.Totale,
    Indirizzo_Spedizione: ordine.IndirizzoSpedizione,
    IsItaly: fsExtra.IsItaly,
    Colli: ordine.Colli,
    Peso: ordine.Peso,
    Articoli: (ordine.Articoli || []).map((a) => ({ SKU: a.Sku, Titolo: a.Titolo, Quantita: a.Quantita })),
    Email: fsExtra.Email,
    GLS_ContractIndex: ordine.GlsContractIndex,
    GLS_IsInternational: fsExtra.GLS_IsInternational,
    GLS_TotalPackages: fsExtra.GLS_TotalPackages,
    GLS_Tracking: fsExtra.GLS_Tracking,
  };
}
...
// Split "prefix/nome.ext" (fileName costruito nei call site sotto, es.
// "ZPL_Labels/123_pkg.zpl") in {subdir, filename} per saveBufferToPrivate.
function splitStoragePath(fileName) {
  const idx = fileName.lastIndexOf("/");
  return idx === -1
    ? { subdir: "gls-labels", filename: fileName }
    : { subdir: fileName.slice(0, idx), filename: fileName.slice(idx + 1) };
}
...
async function getContractIndexFromOrder(ordiniId) {
  try {
    const orderData = await orderDataFromPg(ordiniId);
    if (!orderData) {
...
// Create a b2b.spedizioni row for each newly added parcel
async function createSpedizioniEntries(parcelResults) {
  ...
  for (const pr of parcelResults) {
    // Resolve Source from the order, fallback to 'GLS'
    let sourceVal = "GLS";
    if (pr.orderRef) {
      try {
        const ord = await getOrdine(String(pr.orderRef));
        if (ord) sourceVal = ord.Source || sourceVal;
      } catch (e) {
    ...
    entries.push({
      ...
      ordineId: pr.orderRef || null,
      orderIdExt: pr.bda || null,
  ...
  await pgCreateSpedizioniEntries(entries);
}

// Update status on existing b2b.spedizioni rows
async function updateSpedizioniStatus(resultsArray, newStatus) {
  await pgUpdateSpedizioniStatus(resultsArray, newStatus);
}
...
async function uploadZplToStorage(zplData, fileName) {
  try {
    const { subdir, filename } = splitStoragePath(fileName);
    const url = await saveBufferToPrivate(Buffer.from(zplData, "utf8"), filename, subdir);
...
async function processOrderZpl(ordiniId) {
  try {
    ...
    if (orderData.GLS_ContractIndex === undefined || orderData.GLS_ContractIndex === null) {
    ...
    await updateOrdineGls(ordiniId, {
      corriere: "GLS",
      fsExtraMerge: {
        ZPL_Labels: zplUrls,
  ...
  for (const result of results) {
    if (result.status === "pending") {
      try {
        ...
        await updateOrdineGls(result.orderId, {
          fsExtraMerge: {
            GLS_DeletedAt: new Date().toISOString(),
            GLS_DeleteResults: orderDeleteResults,
            GLS_Status: successCount > 0 ? "parcels_deleted" : "delete_failed",
          },
        });
...
async function processOrderParcels(auth, ordiniId, contractIndex = 0) {
  ...
  const publicUrl = await uploadPdfToStorage(combinedPdfBase64, fileName);
  ...
  await updateOrdineGls(ordiniId, {
    glsTrackingNumber: finalTrackingNumber,
    glsPdfUrl: publicUrl,
    glsContractIndex: contractIndex,
    fsExtraMerge: {
...
async function uploadPdfToStorage(base64PdfData, fileName) {
  try {
    ...
    const url = await saveBufferToPrivate(pdfBuffer, filename, subdir);
...
async function closeOrderParcels(ordiniId) {
  ...
  await updateOrdineGls(ordiniId, {
    stato: successCount > 0 ? "Spedito" : undefined,
    fsExtraMerge: {
      GLS_ClosedAt: new Date().toISOString(),
      GLS_CloseResults: closeResult,
      GLS_Status: successCount > 0 ? "parcels_closed" : "close_failed",
    },
  });

  // Chiudere il manifesto GLS è il vero momento "Spedito" per molti ordini
  // (admin/spedizioni "Trasmetti Spedizioni") — mancava la notifica
  // marketplace qui, stesso bug già trovato/fissato per lo scan-out
  // magazzino e la lista ordini (2026-08-20). pushOrderTracking dispatcha
  // già per Source, nessun filtro necessario qui.
  if (successCount > 0) {
    await processMarketplaceAction({ action: "pushTracking", ordineId: ordiniId, corriere: "GLS" }).catch((err) => {
      console.error(`[gls/sdk closeOrderParcels] pushTracking failed for ${ordiniId}`, err);
    });
  }
      ...
      for (const ordiniId of orderIds) {
        try {
          ...
          await updateOrdineGls(ordiniId, {
            ...
            fsExtraMerge: {
              GLS_ClosedAt: new Date().toISOString(),
              GLS_CloseResults: orderCloseResults,
              GLS_Status: successCount > 0 ? "parcels_closed" : "close_failed",
            },
          });

          // Vedi commento in closeOrderParcels sopra — stesso fix, percorso bulk.
          if (successCount > 0) {
            await processMarketplaceAction({ action: "pushTracking", ordineId: ordiniId, corriere: "GLS" }).catch((err) => {
              console.error(`[gls/sdk closeMultipleOrders] pushTracking failed for ${ordiniId}`, err);
            });
          }

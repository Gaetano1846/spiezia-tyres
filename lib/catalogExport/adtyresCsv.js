...
//
// Stesso identico file (stesso sconto, stesse colonne) caricato anche su
// tyre-world.de (host reale `tyretyre.de`, confermato dal banner FTP "Welcome
// to Tyre-World Data-Exchange" — non un refuso), cartella `/upload/stock/`,
// richiesto 2026-08-07. Il fallimento di QUESTO secondo upload non deve far
// fallire il job intero (AdTyres resta l'integrazione primaria, monitorata
// da anni): viene solo loggato in `errors` → job marcato "done_with_errors".
//
...
export async function runAdtyresCsvExport(opts = {}) {
  ...
  const errors = [];
  try {
    await uploadToTyreworldFtp(csv);
  } catch (err) {

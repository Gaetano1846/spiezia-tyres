...
function transformProductData(data) {
  ...
  // Pirelli / Bridgestone → preferisci Prezzo_Anonimo, fallback prezzo paese
  const prezzoGE = useAnonymous ? (data.Prezzo_Anonimo ?? data.Prezzo_Germania ?? "") : (data.Prezzo_Germania ?? "");
  const prezzoA = useAnonymous ? (data.Prezzo_Anonimo ?? data.Prezzo_Austria ?? "") : (data.Prezzo_Austria ?? "");
  ...
  return {
    ...
    Stock: stock,
    "Price IT": data.Prezzo_Privato ?? "",
    "Price FR": data.Prezzo_Francia ?? "",
    "Price GE": prezzoGE,
    "PRICE A": prezzoA,
    Image: data.Immagine ?? "",

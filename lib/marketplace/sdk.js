...
//   • TyreWorld : TYREWORLD_FTP_HOST/USER/PASSWORD — attenzione, è l'account
//     tyretyre.de (NOSTRE credenziali sul LORO server, dove carichiamo in
//     upload/tracking/), non va confuso con TYREWORLD_ORDERS_FTP_* usato da
//     lib/importers/tyreworld.js (il LORO account sul NOSTRO server, da cui
//     leggiamo gli ordini in ingresso).
//   • 07ZR (Distri2B) : DISTRI2B_API_USERNAME/PASSWORD — REST vera (JWT via
//     /api/login_check), non FTP. distributorId letto per-ordine da
//     FsExtra.ZR07_Distributor (scritto da lib/importers/zr07.js), fallback
//     env DISTRI2B_DISTRIBUTOR_ID. Tracking e stato sono disaccoppiati su
//     richiesta esplicita: addTrackingOrder quando c'è un tracking,
//     ChangeStatusOrder quando cambia lo Stato — mai bundlati insieme come
//     invece succede per Tyre24.
//
// Dispatch per Source dell'ordine, identico al widget FlutterFlow
// spedizioni_admin (Tyre24/Anonimo → Alzura, eBay, Amazon, AdTyres, TyreWorld,
// 07ZR). SDA (Reshark) è stato dismesso — non più supportato come corriere.

import { Client as FtpClient } from "basic-ftp";
import { Readable } from "node:stream";
import { getOrdine } from "../ordiniDb";
...
// ─── Amazon SP-API ───────────────────────────────────────────────────────────
export async function amazonGetToken() {
...
async function amazonFetchOrderItems(accessToken, amazonOrderId) {
...
async function amazonGetOrderItems(accessToken, amazonOrderId) {
  const items = await amazonFetchOrderItems(accessToken, amazonOrderId);
...
// Usato dall'import ordini (lib/importers/woo.js) per risolvere gli articoli
// Amazon dove WooCommerce non collega né SKU né product_id reale (succede per
// una parte non trascurabile delle righe — l'integrazione Amazon->WooCommerce
// a volte non porta un product_id, solo titolo+prezzo). SellerSKU/ASIN qui
// sono invece SEMPRE quelli veri, sono i dati dell'inserzione Amazon stessa.
export async function amazonGetOrderItemsWithSku(accessToken, amazonOrderId) {
  const items = await amazonFetchOrderItems(accessToken, amazonOrderId);
  return items.map((i) => ({
...
async function adTyresTracking({ orderDocId, tracking }) {
  const ordine = await getOrdine(orderDocId);
  if (!ordine) throw new Error(`Order document not found: ${orderDocId}`);

  const adtOrderNumber = ordine.FsExtra?.ADT_OrderNumber;
...
// ─── TyreWorld.de — export stato ordine (schema <bestellstatus> concordato
// col partner, vedi bestellstatus_versendet.xml / bestellstatus_storniert.xml
// scambiati via email). A differenza degli altri marketplace non è un'API
// REST né l'account ordini in ingresso: è un file XML caricato via FTPS
// nella dropbox upload/tracking/ del LORO account (TYREWORLD_FTP_*).
function xmlDate(d) {
  return d.toISOString().slice(0, 19).replace("T", " ");
}

function glsTrackingUrl(tracking) {
  return `https://gls-group.eu/IT/it/servizi-online/ricerca-spedizioni?match=${tracking}`;
}

function buildBestellstatusXml({ bestellnummer, status, sendungen, grund }) {
  const lines = [
    `<?xml version="1.0" encoding="UTF-8"?>`,
    `<bestellstatus>`,
    `  <bestellnummer>${escapeXml(bestellnummer)}</bestellnummer>`,
    `  <statusdatum>${xmlDate(new Date())}</statusdatum>`,
    `  <status>${status}</status>`,
  ];
  if (status === "versendet") {
    lines.push(`  <sendungen>`);
    for (const s of sendungen || []) {
      lines.push(
        `    <sendung>`,
        `      <spediteur><![CDATA[${s.spediteur}]]></spediteur>`,
        `      <trackingnummer>${escapeXml(s.trackingnummer)}</trackingnummer>`,
        `      <trackingurl><![CDATA[${s.trackingurl}]]></trackingurl>`,
        `    </sendung>`
      );
    }
    lines.push(`  </sendungen>`);
  }
  if (status === "storniert" && grund) {
    lines.push(`  <grund><![CDATA[${grund}]]></grund>`);
  }
  lines.push(`</bestellstatus>`, "");
  return lines.join("\n");
}

async function tyreworldSendStatus({ ordineId, status, grund }) {
  const ordine = await getOrdine(ordineId);
  if (!ordine) throw new Error(`Ordine ${ordineId} non trovato`);
  if (ordine.Source !== "TyreWorld") {
    return { source: ordine.Source || "(sconosciuto)", skipped: true, ok: true, detail: "Export stato disponibile solo per ordini TyreWorld" };
  }
  // Difensivo, come per Distri2B sopra: "versendet" (con tracking) va
  // comunicato solo quando l'ordine è davvero Spedito, mai prima. Oggi
  // l'unico chiamante è già gated su questo (handleStatoChange → Spedito),
  // ma il guard evita che un futuro punto di innesco lo mandi in anticipo.
  if (status === "versendet" && ordine.Stato !== "Spedito") {
    return { source: "TyreWorld", skipped: true, ok: true, detail: `Tracking inviato solo quando l'ordine è Spedito (stato attuale: ${ordine.Stato})` };
  }

  const bestellnummer = ordine.FsExtra?.TYREWORLD_OrderNumber || ordineId.replace(/^TYREWORLD-/, "");

  let sendungen;
  if (status === "versendet") {
    const tracking = ordine.GlsTrackingNumber || "";
    if (!tracking) throw new Error("GlsTrackingNumber mancante sull'ordine: impossibile comunicare 'versendet' a TyreWorld");
    sendungen = [{ spediteur: "GLS", trackingnummer: tracking, trackingurl: glsTrackingUrl(tracking) }];
  }

  const xml = buildBestellstatusXml({ bestellnummer, status, sendungen, grund });
  const fileName = `${bestellnummer}_${status}_${Date.now()}.xml`;
  ...
  ftp.ftp.verbose = false;
  try {
    // Plain FTP, non FTPS: il server rifiuta esplicitamente AUTH TLS ("502
    // Explicit TLS authentication not allowed", verificato 2026-08-18 con
    // basic-ftp in verbose — curl --ftp-ssl sembrava funzionare solo perché
    // fa fallback silenzioso al plaintext quando il TLS viene rifiutato,
    // senza --ftp-ssl-reqd). Stesso schema di adTyresTracking sopra.
    await ftp.access({ host, user, password, secure: false });
    await ftp.ensureDir("/upload/tracking");
    await ftp.uploadFrom(Readable.from([xml]), fileName);
  } finally {
    ftp.close();
  }

  return { source: "TyreWorld", ok: true, detail: `Stato '${status}' comunicato a TyreWorld (${fileName})` };
}

// ─── 07ZR (Distri2B) — REST vera, JWT via /api/login_check (RS256, ~1h,
// nessuna cache: un token per chiamata, stesso stile di ebayGetToken/
// amazonGetToken sopra — il volume di chiamate è basso, non serve caching).
const DISTRI2B_API_BASE = "https://api.distri2b.com/api";

async function distri2bGetToken() {
  const username = process.env.DISTRI2B_API_USERNAME;
  const password = process.env.DISTRI2B_API_PASSWORD;
  if (!username || !password) throw new Error("Missing DISTRI2B_API_USERNAME/PASSWORD");
  const res = await httpRequest(`${DISTRI2B_API_BASE}/login_check`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ username, password }),
  });
  if (!res.ok || !res.body?.token) throw new Error(`Distri2B login error ${res.status}: ${res.rawText?.slice(0, 200)}`);
  return res.body.token;
}

// order_number/distributorId sono quelli scritti da lib/importers/zr07.js
// dentro FsExtra (ZR07_OrderNumber, ZR07_Distributor) — stessi valori usati
// dal partner nel suo report "recap-missing-trackings" (verificato).
function distri2bOrderRef(ordine) {
  const orderNumber = ordine.FsExtra?.ZR07_OrderNumber;
  if (!orderNumber) throw new Error("FsExtra.ZR07_OrderNumber mancante sull'ordine: impossibile contattare Distri2B");
  const distributorId = Number(ordine.FsExtra?.ZR07_Distributor) || Number(process.env.DISTRI2B_DISTRIBUTOR_ID) || null;
  if (!distributorId) throw new Error("distributorId mancante (né FsExtra.ZR07_Distributor né DISTRI2B_DISTRIBUTOR_ID)");
  return { orderNumber, distributorId };
}

async function distri2bAddTracking({ ordineId }) {
  const ordine = await getOrdine(ordineId);
  if (!ordine) throw new Error(`Ordine ${ordineId} non trovato`);
  if (ordine.Source !== "07ZR") {
    return { source: ordine.Source || "(sconosciuto)", skipped: true, ok: true, detail: "Tracking Distri2B disponibile solo per ordini 07ZR" };
  }
  // Il tracking GLS esiste già quando l'ordine passa a "In Preparazione"
  // (creazione etichetta, vedi runBulkShipmentJob in app/api/gls-italy/route.ts),
  // ma va comunicato a Distri2B SOLO quando l'ordine è davvero Spedito — mai
  // in anticipo solo perché il numero di tracking è già disponibile.
  if (ordine.Stato !== "Spedito") {
    return { source: "07ZR", skipped: true, ok: true, detail: `Tracking Distri2B inviato solo quando l'ordine è Spedito (stato attuale: ${ordine.Stato})` };
  }
  const tracking = ordine.GlsTrackingNumber || "";
  if (!tracking) throw new Error("GlsTrackingNumber mancante sull'ordine: impossibile comunicare il tracking a Distri2B");

  const { orderNumber, distributorId } = distri2bOrderRef(ordine);
  const token = await distri2bGetToken();
  const res = await httpRequest(`${DISTRI2B_API_BASE}/addTrackingOrder`, {
    method: "POST",
    headers: { Authorization: `Bearer ${token}`, "Content-Type": "application/json" },
    body: JSON.stringify({
      distributorId,
      orders: [{
        ...
        tracking_link: glsTrackingUrl(tracking),
        // Non abbiamo un "bon de livraison" nostro distinto dal tracking
        // GLS — vuoto finché una risposta reale non ci dice se è
        // obbligatorio e cosa aspettarsi (vedi commento su tyreworldSendStatus
        // per lo stesso approccio "verifica sul primo invio reale").
        delivery_number: "",
        carrier: "GLS",
      }],
    }),
  });
  const ok = res.ok && res.body?.code === "OK";
  return { source: "07ZR", ok, detail: ok ? "Tracking comunicato a Distri2B" : `Distri2B addTrackingOrder ${res.status}: ${res.rawText?.slice(0, 200)}` };
}

async function distri2bChangeStatus({ ordineId, newStatus, comment }) {
  const ordine = await getOrdine(ordineId);
  if (!ordine) throw new Error(`Ordine ${ordineId} non trovato`);
  if (ordine.Source !== "07ZR") {
    return { source: ordine.Source || "(sconosciuto)", skipped: true, ok: true, detail: "Stato Distri2B disponibile solo per ordini 07ZR" };
  }
  const { orderNumber, distributorId } = distri2bOrderRef(ordine);
  const token = await distri2bGetToken();
  const res = await httpRequest(`${DISTRI2B_API_BASE}/ChangeStatusOrder`, {
    method: "POST",
    headers: { Authorization: `Bearer ${token}`, "Content-Type": "application/json" },
    body: JSON.stringify({ distributorId, orders: [{ order_number: orderNumber, new_status: newStatus, comment: comment || "" }] }),
  });
  const orderResult = res.body?.orders?.[0];
  const ok = res.ok && res.body?.code === "OK" && (orderResult?.return_message?.startsWith("OK") ?? true);
  const detail = ok
    ? `Stato '${newStatus}' comunicato a Distri2B${orderResult?.return_message ? ` (${orderResult.return_message})` : ""}`
    : `Distri2B ChangeStatusOrder ${res.status}: ${orderResult?.return_message || res.rawText?.slice(0, 200)}`;
  return { source: "07ZR", ok, detail };
}
...
async function pushOrderTracking({ ordineId, corriere }) {
  ...
  const source = ordine.Source || "";
  const glsTracking = ordine.GlsTrackingNumber || "";
  const courierCode = corriere === "SDA" ? "SDA" : "GLS";

  if (source === "Tyre24" || source === "Anonimo") {
    const country = ordine.T24Country || "";
  ...
  if (source === "eBay") {
    const ebayOrderId = ordine.FsExtra?.eBay_OrderID || "";
  ...
  if (source === "Amazon") {
    const marketplaceId = ordine.FsExtra?.Amazon_MarketplaceID || "";
    const wcOrderNumber = ordine.FsExtra?.WC_OrderNumber || "";
  ...
  if (source === "TyreWorld") {
    return tyreworldSendStatus({ ordineId, status: "versendet" });
  }

  if (source === "07ZR") {
    return distri2bAddTracking({ ordineId });
  }
...
export async function processMarketplaceAction(body) {
  ...
  try {
    switch (action) {
      ...
      case "tyreworldStatus": {
        if (!params.ordineId) return { statusCode: 400, payload: { error: "ordineId richiesto" } };
        if (!["versendet", "storniert", "in_bearbeitung"].includes(params.status)) {
          return { statusCode: 400, payload: { error: "status deve essere 'versendet', 'storniert' o 'in_bearbeitung'" } };
        }
        const result = await tyreworldSendStatus({ ordineId: params.ordineId, status: params.status, grund: params.grund });
        return { statusCode: 200, payload: { success: true, data: result } };
      }
      case "distri2bStatus": {
        if (!params.ordineId) return { statusCode: 400, payload: { error: "ordineId richiesto" } };
        const DISTRI2B_STATUSES = [
          "ORDER_VALIDATED", "ORDER_CANCELALL", "ORDER_DENIEDDATA", "ORDER_DENIEDOTHER",
          "ORDER_DENIEDUNPAYED", "ORDER_UNPAYED", "ORDER_BLOCKED_PREPAYEMENT",
          "ORDER_UNPAYED_REGULATED", "UNPAYED_SUPPORTED", "SENT",
        ];
        if (!DISTRI2B_STATUSES.includes(params.status)) {
          return { statusCode: 400, payload: { error: `status deve essere uno tra: ${DISTRI2B_STATUSES.join(", ")}` } };
        }
        const result = await distri2bChangeStatus({ ordineId: params.ordineId, newStatus: params.status, comment: params.comment });
        return { statusCode: 200, payload: { success: true, data: result } };
      }
      default:

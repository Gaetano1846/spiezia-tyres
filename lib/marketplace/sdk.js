...
async function tyreworldSendStatus({ ordineId, status, grund }) {
  ...
  ftp.ftp.verbose = false;
  try {
    // Richiede FTPS esplicito, certificato valido (verificato 2026-08-18,
    // stesso host tyretyre.de usato per export stock/listini).
    await ftp.access({ host, user, password, secure: true });
